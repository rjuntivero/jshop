      ...
      <div className="">
        <Link href={`/products/${product?.id}`}>
          <div className={`relative w-full aspect-square mx-auto overflow-hidden rounded-lg transition-opacity duration-500 ${loaded ? 'opacity-100' : 'opacity-0'}`}>
            <Image sizes="(max-width: 768px) 100vw, 400px" src={product!.image!} alt={product?.title ?? 'Product Image'} fill className="object-contain object-center p-1 md:p-4" onLoad={() => setLoaded(true)} />
          </div>
        </Link>

        <div className="flex flex-col p-1 md:p-3 bg-white  border-t-1 border-t-primary-light/20">
          <h1 className="font-sub-header text-primary-light dark:text-secondary-dark truncate  text-xl md:text-xl">{product?.title}</h1>
