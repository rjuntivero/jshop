    ...
    <main className="text-primary-light dark:bg-background-dark flex items-center gap-12 px-6">
      <div className="flex w-full items-center gap-8">
        <Button
          ...
          className={`group hover:bg-secondary-light/40 text-secondary-light dark:bg-background-dark dark:text-accent-dark border-secondary-light dark:border-accent-dark relative hidden items-center justify-center px-4 py-2 text-[0.75rem] transition-all duration-300 md:flex ${dropdown ? 'bg-secondary-light/40' : ''}`}
        >
          Sort By
          <Dropdown />
          <div
            className={`absolute bottom-0 flex translate-y-full bg-white p-4 shadow-md transition-all duration-150 ease-in-out ${dropdown ? 'translate-x-0 opacity-100 focus-within:pointer-events-auto' : '-translate-x-3 opacity-0 not-focus-within:pointer-events-none'}`}
          >
            <ul
              className={`grid items-center justify-center gap-3 overflow-hidden transition-all duration-300 ${dropdown ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}
            >
              {filters.map((filter) => (
                <li
                  className="hover:bg-secondary-light/40 w-full cursor-pointer p-4 py-2 text-center font-bold transition-all duration-200"
                  key={filter}
                >
                  {filter}
                </li>
