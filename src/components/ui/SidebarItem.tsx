...
const SidebarItem: React.FC<ItemProps> = ({ item, isActive, handleItemClick }) => {
  return (
    <li key={item}>
      <Button onClick={() => handleItemClick(item)} className="group relative mb-4 w-full rounded-full px-4 py-2 text-left font-bold tracking-widest transition-colors duration-300 ease-in-out">
        <span
